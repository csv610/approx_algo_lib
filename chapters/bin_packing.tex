...
The guarantee for Next-Fit is the absolute bound $\text{NF}(I) \le 2\,\text{OPT}(I) + 1$ (any two consecutive Next-Fit bins have total size $> 1$). A common but \emph{incorrect} attempt at a tight example pairs items of sizes $0.5+\delta$ and $0.5-\delta$; those always fit together exactly (sum $= 1$), so they pack into the same number of bins under both Next-Fit and OPT. That instance has ratio 1, not 2.

A correct example showing Next-Fit loses a constant factor is obtained from two sizes $a = \frac38$ and $b = \frac58$, with $a + b = 1$. Take $n$ items of size $a$ followed by $n$ items of size $b$.
Next-Fit packs the $a$'s two per bin (three would exceed capacity), using $\lceil n/2 \rceil$ bins; each such bin is left with $2/8 < b$ free, so none of the $b$'s fits into the current open bin and each $b$ receives its own bin ($n$ bins). The optimal packing pairs every $a$ with a $b$ into exactly $n$ bins. Thus
\[
\frac{\text{NF}(I)}{\text{OPT}(I)} = \frac{n/2 + n}{n} = \frac32 ,
\]
so Next-Fit can be a factor $3/2$ worse. Its true worst case reaches $2$ only as a limit; the absolute guarantee above remains the correct statement of its performance.

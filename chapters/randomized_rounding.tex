...
For small clauses, this bound is much higher (e.g. $k=1$ gives $1 \cdot z_j$, $k=2$ gives $0.75 \cdot z_j$). For large clauses, the coin-flip baseline satisfies them with probability $1 - 2^{-k}$. Choosing the best of both algorithms yields a $3/4$-approximation (Goemans and Williamson~\cite{goemans-williamson94}).
...
Single-literal clauses are easy: for a unit clause $(x_i)$ the LP sets $y_i^* = 1$ and $z_i^* = 1$, and randomized rounding sets $x_i = 1$ with probability 1, satisfying it exactly. So unit clauses do not stress the bound.

The gap is driven by \emph{large} clauses. For a clause of size $k$, the LP rounding guarantee is the factor
\[
1 - \left(1 - \frac{1}{k}\right)^k \;\xrightarrow[k\to\infty]{}\; 1 - \frac1e \approx 0.632 .
\]
A family of $k$-clauses with $k$ large makes this factor the binding one, so the $1-1/e$ ratio is tight for pure LP rounding in the limit of large clauses. The coin-flip baseline, by contrast, satisfies a $k$-clause with probability $1 - 2^{-k} \to 1$, which is what lets the combination of the two (choosing the better) escape the $\approx 0.632$ ceiling and reach $3/4$.

...
\begin{theorem}[Christofides~\cite{christofides76}]
...
Iterative rounding & $< 1.5$ & Recent breakthroughs (Karlin et al.~\cite{karlin21}: 1.5 - $\epsilon$) \\\\
\bottomrule
\end{tabular}
\end{table}

\textbf{Why not exact?} Metric TSP is NP-hard (reduction from Hamiltonian Cycle). The best exact algorithm is $O(n^2 2^n)$ (Held-Karp DP~\cite{held-karp62}).

\textbf{Why 3/2 was a barrier?} For 45 years no improvement. Karlin, Klein, and Gharan~\cite{karlin21} broke it with $1.5 - 10^{-36}$, later improved to $1.5 - \epsilon$.
...
\textbf{Model:} Complete graph on 51 nodes (depot + 50 customers). Edge costs = travel time (satisfies triangle inequality). Optimal TSP tour = optimal single-truck route. For 5 trucks: split TSP tour into 5 segments (each $\leq$ capacity). Metric TSP 1.5-approx (Christofides~\cite{christofides76}) gives route within $1.5 \times$ optimal single-truck distance.
\textbf{Why approximation matters:} Exact TSP (Held-Karp DP~\cite{held-karp62}) is $O(n^2 2^n)$ --- impossible for $n=50$. Christofides runs in $O(n^3)$ and gives provable $1.5\times$ bound. In practice, Christofides + 2-opt local search often achieves $< 1.1\times$ optimal.
...
Consider the $n$-vertex cycle $C_n$ with every edge of cost 1 (this is a metric). The MST is the $n-1$-edge path obtained by dropping one edge, so $\text{cost}(MST) = n-1$. Doubling every edge gives an Eulerian tour of cost $2 \cdot (n-1)$. The optimal TSP tour is simply going around the cycle once, costing $n$. Ratio $= \frac{2(n-1)}{n} \to 2$.
\end{example}

\begin{example}[Christofides]
An important point about the bound: on the $n$-cycle of unit-cost edges, the minimum spanning tree is a path whose only odd-degree vertices are its two endpoints. Those endpoints are adjacent via the single dropped cycle edge, so the min-weight perfect matching on the odd set has cost $1$ (not $(n+1)/2$). Christofides then returns a tour of cost $(n-1) + 1 = n = \text{OPT}$, i.e. the algorithm is optimal here. The factor-$3/2$ guarantee is a worst case, attained only on instances where the odd-set matching contributes roughly $\text{OPT}/2$ in addition to an $\text{OPT}$-sized spanning tree; a plain uniform cycle does not exhibit this.
\end{example}

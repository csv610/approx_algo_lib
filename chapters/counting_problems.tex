...
\textbf{Monte Carlo Estimation:} We can approximate $\#$P-hard counts using Monte Carlo sampling. The key is to design an efficient sampler that produces estimates with low variance. The Karp-Luby algorithm for DNF counting achieves this by importance sampling: pick a random clause, sample a satisfying assignment uniformly, and use the ratio estimator.
\end{intuitionbox}
...
\subsection{Karp-Luby for DNF Counting}
\begin{enumerate}
...
\begin{lstlisting}[style=python, caption=Karp-Luby DNF Counting]
import random
